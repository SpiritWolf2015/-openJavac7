...
/**
 * 符号基类，符号表的单个元素。所谓“符号”意思是表示程序元素，如程序包，类或方法。 每个元素表示静态的语言级结构（而不是虚拟机的运行时结构）。 <p>
...
public abstract class Symbol implements Element {
    ...
    /**
     * kind保存符号的具体类型，允许的值已经在Kinds类中预先进行了定义<p>
    ...
    /**
     * flags_field保存符号的修饰符，允许取的值已经在Flags类中预先进行了定义。<p>
    ...
    /**
     * name用来保存符号名称，通常就是在声明或定义时指定的唯一标识符<p>
    ...
    /**
     * owner保存当前符号所属的符号，也就是在owner中定义了当前的符号<p>
     * The owner of this symbol.
     */
    public Symbol owner;

    /**
     * completer与符号表的输入密切相关<p>
    ...
    public static class VarSymbol extends Symbol implements VariableElement {
        ...
        /**
         * adr在数据流分析、语法糖去除与代码生成阶段代表了不同的含义。<p>
...
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
        ...
        /**
         * 如果VarSymbol对象表示常量，则data存储常量数值，如果表示变量，则存储java.util.concurrent.Callable对象，这样可以延迟处理变量初始化表达式。<p>
    ...
    /**
     * 任何方法，包括接口、抽象类或者注解中的方法都有一个唯一的MethodSymbol对象。<p>
    ...
    public static class MethodSymbol extends Symbol implements ExecutableElement {
        ...
        /**
         * 查找方法实现，在语义分析阶段会调用implementation()方法检查类中定义或继承下来的所有抽象方法是否都有对应的实现<p>
         * The implementation of this (abstract) symbol in class origin;
    ...
    /**
     * OperatorSymbol类表示的是运算符符号，任何在Java源代码中出现的运算符都有一个唯一的OperatorSymbol对象。
     * Javac将任何运算符的操作数看作调用方法传递的参数，将运算后的结果当作调用方法后的返回值。不过这里指的运算仅仅是对类型的运算，
     * 例如，两数相加的运算符“+”，其中一个操作数为long类型，另外一个为int类型，那么Javac就会查找名称为“+”并且参数类型为long和int的OperatorSymbol对象，
     * 然后获取这个对象的结果类型。<p>
