...
public class Symtab {
    ...
    protected Symtab(Context context) throws CompletionFailure {
        ...
        // 预先进行运算符的重载，也就是为不同的运算符、相同运算符不同类型的操作数创建唯一的OperatorSymbol对象，如表6-5所示。
        ...
        // Enter operators.
        // 调用enterUnop()方法为一元运算符创建OperatorySymbol对象，每调用一次enterUnop()方法就会创建一个sym，这个对象的name为“+”,
        // arg就是操作数的类型，这里可以理解为方法的形参类型，而res是运算的结果类型，在这里可以理解为方法返回值类型。
        // 通过arg与res两个参数创建了MethodType对象，MethodType表示方法类型
