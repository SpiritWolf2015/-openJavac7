...
/**
 * Type类是Javac中所有类型的父类，通过类中定义的变量tag可以确定具体的类型，其值已经在TypeTags枚举类中预先进行了定义。由于Type类可以表示基本类型，
 * 因此这个类实现了PrimitiveType接口，对应的tag值可以是BYTE、CHAR、SHORT、INT、LONG、FLOAT、DOUBLE或BOOLEAN，
 * 剩下的引用类型及一些特殊的类型由Type类或Type类的子类来表示，子类都以静态内部类的形式定义在Type类中。
 * Type类及相关的子类用来进行类型验证，尤其是在表达式的类型验证过程中起着至关重要的作用。它与符号一起对表达式的合法性进行检查，一般都是先查找符号，
 * 然后再通过符号查找具体的类型，因此在Symbol类中定义了变量type，用来对具体的类型进行保存。<p>
 *
...
public class Type implements PrimitiveType {
    ...
    /**
     * tsym变量表示当前Type对象是被哪个TypeSymbol对象定义的，具体可能为TypeSymbol、ClassSymbol或PackageSymbol对象。对于引用类型来说，
     * 通常在创建Type类或子类的对象时会调用构造方法初始化tsym。对于基本类型来说，表示基本类型的Type对象的tsym值一般都为null。<p>
    ...
    */
    ...
    /**
     * WildcardType表示通配符类型。例如，声明一个参数化类型List<? extends Number>，其中的？ extends Number就是用WildcardType对象来表示。
     * type保存了通配符类型的上界或下界；kind保存了具体通配符的类型；bound保存了类型定义时声明的类型变量，一般为TypeVar对象。
     * 具体的通配符类型由kind来指定，值已经在BoundKind枚举类中预先进行了定义。
     */
    public static class WildcardType extends Type implements javax.lang.model.type.WildcardType {
        ...
        public Type type;
        /**
         * kind保存了具体通配符的类型；值已经在BoundKind枚举类中预先进行了定义。
         */
    ...
    /**
     * ClassType类表示类型，可能为类或接口。ClassType实现了DeclaredType接口，表示声明的类或接口，tag值为CLASS, tsym值一般为ClassSymbol对象。
     * 除了这两个继承自父类Type的变量外，ClassType类中还定义了许多变量，outer_field保存封闭类型，如果当前类是非内部类，
     * 这个值为Type类中预先定义的、类型为JCNoType的noType对象；typarams_field保存了类型参数的类型，如果当前表示的是定义时的类型，
     * 则typarams_field保存的是形式类型参数的类型；如果是使用已定义的类型，则typarams_field保存的是实际类型参数的类型；allparams_field除了保存所有类型参数的类型外，
     * 还会保存宿主类型中的所有类型参数；supertype_field保存当前类型的父类，如果为接口，则supertype_field保存的是Object类型；interfaces_field保存当前类型实现的所有接口。
     */
...
//where
    ...
    /**
     * 继承ClassType类的ErasedClassType类用来辅助查找泛型类型的超类
     */
    ...
    /**
     * UnionClassType类用来支持在单个catch语句中声明多个异常捕获类型<p>
    ...
    public static class UnionClassType extends ClassType implements UnionType {
        // alternatives_field列表保存了catch语句中声明多个异常捕获的类型
    ...
    public static class ArrayType extends Type
        ...
        /**
         * elemtype表示组成数组元素的类型。elemtype的类型也可能是ArrayType，例如一个二维数组
         */
    ...
    public static class MethodType extends Type implements ExecutableType {
        // 变量argtypes、restype与thrown分别保存了形式参数类型、返回值类型及抛出的异常类型
    ...
    /**
     * PackageType类用来表示包类型，由于PackageType不表示任何具体的类型，因此实现了NoType接口。tag值为PACKAGE, tsym值为PackageSymbol对象。
     */
    ...
    /**
     * TypeVar表示类型变量，bound保存类型变量的上界，而lower保存类型变量的下界。由于类型变量在声明时不能指定下界，因而值通常为syms.botType，
     * 也就是null，lower变量的主要作用就是辅助进行类型捕获转换。
     */
    ...
    /**
     * 继承TypeVar类的CapturedType类用来辅助进行类型捕获<p>
    ...
    /**
     * ForAll类用来表示含有泛型变量声明的方法类型，主要辅助进行类型推断
     */
    ...
    /**
     * UndetVar类表示待推断的类型变量，主要辅助进行类型推断<p>
    ...
    /**
     * JCNoType对象的tag值可能为NONE或者VOID，当为NONE表示这不是一个类型，例如包名没有具体的Type类型。
     * 当方法返回类型为void时，tag的值为VOID。<p>
    ...
    public static class ErrorType extends ClassType
        ...
        // originalType变量保存了一个具体的错误类型，用于出错时错误的处理
