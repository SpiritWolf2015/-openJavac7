...
/**
 * 根据Token序列来获取语法树节点信息，并最终转换为一棵完整的抽象语法树。在JSL的第18章又给出了另外一种文法，这种文法并不是严格意义上的LL(1)文法，
 * 虽然文法还有左公因式，但是已经没有了左递归。Javac在实现时如果读取一个Token对象无法确定对应的文法产生式时，还可以继续读取下一个Token对象，
 * 直到确定为止，因此这种文法有利于程序实现。<p>
...
/* ---------- parsing -------------- */
    ...
    /**
     * 解析标识符（标识符泛指用户自定义的类名、包名、变量名、方法名等）<p>
     * Ident = IDENTIFIER
     */
    Name ident() {
...
//where
    ...
    /**
     * 块是由花括号和BlockStatements组成，调用block()方法分析Block<p>
...
//todo: skip to anchor on error(?)
    ...
    /**
     * 读取注解，同时也能读取修饰符（public/protected/private、static、final等）<p>
    ...
    /**
     * 分析编译单元，
     * CompilationUnit文法如下：<p>
     * CompilationUnit = [ { "@" Annotation } PACKAGE Qualident ";"] {ImportDeclaration} {TypeDeclaration}<p>
     * 首先是可能出现0次或1次的包名，然后是可以出现0次或多次的ImportDeclaration和TypeDeclaration。
     * 不过，所有的ImportDeclaration需要在TypeDeclaration之前，包声明之后（如果有包声明的话）。<p>
     */
    public JCTree.JCCompilationUnit parseCompilationUnit() {
        ...
        if (S.token() == PACKAGE) { // 解析包
            ...
            S.nextToken();
            pid = qualident(); // 解析包名
            accept(SEMI); // 处理包声明后的分号，这样包声明就处理完了
        }

        // 处理ImportDeclaration和TypeDeclaration，由于这两个结构都可以出现0次或多次，需要循环多次进行处理
        ListBuffer<JCTree> defs = new ListBuffer<>();
        boolean checkForImports = true;
        // 循环读取Token序列中的所有Token对象，直到读取到EOF为止
        while (S.token() != EOF) {
            if (S.pos() <= errorEndPos) {
                // error recovery
            ...
            if (checkForImports && mods == null && S.token() == IMPORT) {
                defs.append(importDeclaration()); // importDeclaration()解析导入声明
            } else {
                JCTree def = typeDeclaration(mods); // typeDeclaration()解析类型声明
    ...
    JCTree typeDeclaration(JCModifiers mods) {
        ...
        if (mods == null && S.token() == SEMI) {
            ...
            String dc = S.docComment();
            return classOrInterfaceOrEnumDeclaration(modifiersOpt(mods), dc); // 解析类和接口
        }
    }
    ...
    JCStatement classOrInterfaceOrEnumDeclaration(JCModifiers mods, String dc) {
        ...
        } else if (S.token() == INTERFACE) { // 解析接口、注解类
    ...
    JCClassDecl classDeclaration(JCModifiers mods, String dc) {
        ...
        if (S.token() == IMPLEMENTS) { // 解析实现的接口
            S.nextToken();
            implementing = typeList();
        }

        List<JCTree> defs = classOrInterfaceBody(name, false); // 解析类、接口的body体
    ...
    /**
     * 解析类、接口的body体，文法如下：<p>
     * ClassBody     = "{" {ClassBodyDeclaration} "}"<p>
     * InterfaceBody = "{" {InterfaceBodyDeclaration} "}"<p>
     * 最外层的花括号表示终结符，而内层的花括号则表示ClassBodyDeclaration可以出现0次或多次
     */
    List<JCTree> classOrInterfaceBody(Name className, boolean isInterface) {
        ...
        while (S.token() != RBRACE && S.token() != EOF) { // 解释类型的body体
            // 调用classOrInterfaceBodyDeclaration()方法对类或接口中的成员进行解析
    ...
    List<JCTree> classOrInterfaceBodyDeclaration(Name className, boolean isInterface) {
        ...
        pos = S.pos();
        List<JCTypeParameter> typarams = typeParametersOpt();
        // if there are type parameters but no modifiers, save the start position of the method in the modifiers.
        ...
        if (S.token() == LPAREN && !isInterface && type.getTag() == JCTree.IDENT) {
            ...
            return List.of(methodDeclaratorRest(pos, mods, null, names.init, typarams, true, dc));
        }

        pos = S.pos();
        name = ident();
        if (S.token() == LPAREN) {
            return List.of(methodDeclaratorRest(pos, mods, type, name, typarams, isVoid, dc));
        }

        if (!isVoid && typarams.isEmpty()) {
            List<JCTree> defs = variableDeclaratorsRest(pos, mods, type, name, isInterface, dc, new ListBuffer<JCTree>()).toList();
        ...
        List<JCTree> err = isVoid ?
                List.<JCTree>of(toP(F.at(pos).MethodDef(mods, name, type, typarams, List.<JCVariableDecl>nil(), List.<JCExpression>nil(), null, null)))
                : null;
     ...
     */
    JCTree methodDeclaratorRest(int pos, JCModifiers mods, JCExpression type, Name name, List<JCTypeParameter> typarams,
                                boolean isVoid, String dc) {
        List<JCVariableDecl> params = formalParameters(); // 解析方法中的形式参数
        if (!isVoid) type = bracketsOpt(type); // 处理可选的括号部分

        List<JCExpression> thrown = List.nil();
        if (S.token() == THROWS) { // 解析方法抛出的异常
            S.nextToken();
            thrown = qualidentList(); // 处理异常抛出部分
        }

        JCBlock body = null;
        JCExpression defaultValue;
        if (S.token() == LBRACE) { // 解析方法的body体
            body = block(); // 处理代码块
            defaultValue = null;
        } else {
            if (S.token() == DEFAULT) {
        ...
        JCMethodDecl result = toP(F.at(pos).MethodDef(mods, name, type, typarams, params, thrown, body, defaultValue));
        attach(result, dc);
        return result;
    }
    ...
    JCTypeParameter typeParameter() {
        ...
        if (S.token() == EXTENDS) { // 解释类型参数中声明的上界
            ...
            while (S.token() == AMP) {
                S.nextToken();
                // 调用parseType()方法分析形式类型参数的上界，由于上界可能有多个，因此需要循环调用parseType()方法进行分析
                bounds.append(parseType());
            }
        }
