...
/**
 * 每次进行Java源代码编译时，Javac都会生成一个特定的上下文对象com.sun.tools.javac.util.Context，
 * 一些重要类在此上下文中都有唯一的对象，也就是常说的单例。
 * 调用context.put()方法存储了当前对象，这样下次就可以直接重用这个对象了。
 * 通过Context类获取对象，可以达到两个目的：
 * （1）延迟创建相关类的对象；
 * （2）在特定的上下文对象Context中保证某些类的对象的唯一性。
...
public class Context {
    ...
    /**
     * KEY是Key<T>类型，而value存储的是T类型的对象，也可能是Factory<T>类型的工厂对象。
